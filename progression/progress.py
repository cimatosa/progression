...
class PipeFromProgressToIPythonHTMLWidget(object):
    ...
    def __call__(self, b):
        ...
        if b.endswith(terminal.ESC_MY_MAGIC_ENDING):
            ...
            self.htmlWidget.value = (
                "<style>.widget-html{font-family:monospace}</style><pre>"
                + buff
                + "</pre>"
            )
            self._buff = ""
...
def choose_pipe_handler(kind="print", color_theme=None):
    ...
    if kind == "print":
        ...
        if _IPYTHON:
            ...
            warnings.warn(
                "can not choose ipythonHTML (IPython and/or ipywidgets were not loaded)"
            )
    else:
        raise ValueError(
            "unknown kind for pipe handler '{}'. Choices are 'print' and 'ipythonhtml'".format(
                kind
            )
        )
...
def _loop_wrapper_func(
    ...
    log = logging.getLogger(
        __name__ + ".log_{}".format(get_identifier(name, bold=False))
    )
        ...
        try:
            ...
            if shared_mem_pause.value:
                ...
                try:
                    ...
                    log.error(
                        "error %s occurred in loop calling 'func(*args)'", type(e)
                    )
...
class Loop(object):
    ...
    def start(self, timeout=None):
        ...
        self._proc = mp.Process(target=_loop_wrapper_func, args=args)
        self._proc.start()
        ...
        while not self._func_running.value:
            if self._proc.exitcode is not None:
                ...
                if exc == 0:
                    log.warning(
                        "wrapper function already terminated with exitcode 0\nloop is not running"
                    )
                    return
                else:
            ...
            if (timeout is not None) and ((time.time() - t0) > timeout):
                err_msg = "could not bring up function on time (timeout: {}s)".format(
                    timeout
                )
                log.error(err_msg)
                log.info(
                    "either it takes too long to spawn the subprocess (increase the timeout)\n"
...
def show_stat_base(
    count_value, max_count_value, prepend, speed, tet, ttg, width, **kwargs
):
...
def _show_stat_wrapper_Progress(
    count,
    last_count,
    start_time,
    max_count,
    ...
    calculate
    """
    count_value, max_count_value, speed, tet, ttg, = Progress._calc(
        count, last_count, start_time, max_count, speed_calc_cycles, q, last_speed, lock
    )
    return show_stat_function(
        count_value, max_count_value, prepend, speed, tet, ttg, width, i, **add_args
    )


def _show_stat_wrapper_multi_Progress(
    count,
    last_count,
    start_time,
    max_count,
    ...
    print(
        terminal.ESC_MOVE_LINE_UP(n + emtpy_lines_at_end)
        + terminal.ESC_MY_MAGIC_ENDING,
        end="",
    )
    sys.stdout.flush()
    ...
    def __init__(
        ...
        try:
            ...
            if not isinstance(count, Synchronized):
                raise ValueError(
                    "'count' must be if the type multiprocessing.sharedctypes.Synchronized"
                )
            self.is_multi = False
            count = [count]
...
def show_stat_ProgressBar(
    count_value, max_count_value, prepend, speed, tet, ttg, width, i, **kwargs
):
    if (max_count_value is None) or (max_count_value == 0):
        # only show current absolute progress as number and estimated speed
        print(
            "{}{}{} [{}] {}#{}    ".format(
                terminal.ESC_NO_CHAR_ATTR,
...
def show_stat_ProgressBarCounter(
    count_value, max_count_value, prepend, speed, tet, ttg, width, i, **kwargs
):
...
def get_d(s1, s2, width, lp, lps):
    d = (
        width
        - len(terminal.remove_ESC_SEQ_from_string(s1))
        - len(terminal.remove_ESC_SEQ_from_string(s2))
        - 2
        - lp
        - lps
    )
...
def _stat(count_value, max_count_value, prepend, speed, tet, ttg, width, i, **kwargs):
    if (max_count_value is None) or (max_count_value == 0):
        ...
        if ttg is None:
            ...
            eta = datetime.datetime.fromtimestamp(time.time() + ttg).strftime(
                "%Y%m%d_%H:%M:%S"
            )
            ort = tet + ttg
...
def show_stat_ProgressBarFancy(
    count_value, max_count_value, prepend, speed, tet, ttg, width, i, **kwargs
):
    stat = _stat(
        count_value, max_count_value, prepend, speed, tet, ttg, width, i, **kwargs
...
def show_stat_ProgressBarCounterFancy(
    count_value, max_count_value, prepend, speed, tet, ttg, width, i, **kwargs
):
    counter_count = kwargs["counter_count"][i]
    counter_speed = kwargs["counter_speed"][i]
...
class SIG_handler_Loop(object):
    ...
    def __init__(self, sigint, sigterm, log, prefix):
        ...
        self.log = log
        self.log.info(
            "setup signal handler for loop (SIGINT:%s, SIGTERM:%s)", sigint, sigterm
        )
...
def check_process_termination(proc, prefix, timeout, auto_kill_on_last_resort=False):
    ...
    if not proc.is_alive():
        log.debug(
            "termination of process (pid %s) within timeout of %s SUCCEEDED!",
            proc.pid,
            humanize_time(timeout),
        )
        return True

    # process still runs -> send SIGTERM -> see what happens
    log.warning(
        "termination of process (pid %s) within given timeout of %s FAILED!",
        proc.pid,
        humanize_time(timeout),
    )
    ...
    if not proc.is_alive():
        log.info(
            "termination of process (pid %s) via SIGTERM with timeout of %s SUCCEEDED!",
            proc.pid,
            humanize_time(new_timeout),
        )
        return True
    log.warning(
        "termination of process (pid %s) via SIGTERM with timeout of %s FAILED!",
        proc.pid,
        humanize_time(new_timeout),
    )
...
_colthm_term_default = {
    "PRE_COL": terminal.ESC_RED,
    "BAR_COL": terminal.ESC_LIGHT_GREEN,
    "ADD_LNS_UP": 0,
}
_colthm_ipyt_default = {
    "PRE_COL": terminal.ESC_RED,
    "BAR_COL": terminal.ESC_LIGHT_BLUE,
    "ADD_LNS_UP": 0,
}
_colthm_wincmd_default = {
    "PRE_COL": terminal.ESC_RED,
    "BAR_COL": terminal.ESC_GREEN,
    "ADD_LNS_UP": 1,
}

color_themes = {
    "term_default": _colthm_term_default,
